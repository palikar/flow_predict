...
+ _Structural similarity index_ (SSIM) - the metric again measures the similarity between two images. What sets is apart from the other ones is that SSIM is a perception-based model. This means that it respects how a human observer will perceive the differences between the two images.
